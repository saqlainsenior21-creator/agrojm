...
app.use(cors({
  origin: process.env.NODE_ENV === 'production'
    ? [process.env.FRONTEND_URL || 'https://agrojm.com', 'https://www.agrojm.com']
...
app.get('/api/health', (req, res) => {
  try {
    const db = require('./db');
    const { listings } = db.prepare("SELECT COUNT(*) as listings FROM listings").get();
    const { farmers } = db.prepare("SELECT COUNT(*) as farmers FROM users WHERE role='farmer'").get();
    res.json({ status: 'ok', listings, farmers, ts: new Date().toISOString() });
  } catch (e) {
...
app.listen(PORT, () => logger.info(`🌱 AgroJM running on port ${PORT}`));
